...
function modify_ca_server_tls {
    ...
    if command -v yq &> /dev/null; then
        ...
        log "yq not found. Applying fallback config patch..."
                sed -i "s|^\([[:space:]]*name:[[:space:]]*\).*|\1${CA_NAME}|" "$CA_SERVER_TLS_CONFIG"
                sed -i '/^tls:/,/^[^[:space:]]/ s|^\([[:space:]]*enabled:[[:space:]]*\).*|\1true|' "$CA_SERVER_TLS_CONFIG"
                sed -i "/^csr:/,/^[^[:space:]]/ s|^\([[:space:]]*hosts:[[:space:]]*\).*|\1\n    - ${CSR_HOSTS}|" "$CA_SERVER_TLS_CONFIG"

                if ! grep -q "### custom tls profile ###" "$CA_SERVER_TLS_CONFIG"; then
                        cat >> "$CA_SERVER_TLS_CONFIG" <<EOF
...
EOF
        ...
        # remove old initialized certificates
        rm -f "${CA_SERVER_TLS_DIR}/ca-cert.pem"
        rm -rf "${CA_SERVER_TLS_DIR}/msp"
}
